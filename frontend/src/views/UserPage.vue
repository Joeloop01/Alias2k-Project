...
async function onNewTodo(newTodo: NewTodo) {
  closeNewModal()
  await postTodo(id, newTodo)
  await getAllTodos(id).then((d) => (todosData.value = d))
}

async function onEditTodo(newTodo: NewTodo, todoId: string) {
  closeEditModal()
  await putTodo(id, newTodo, todoId)
  await getAllTodos(id).then((d) => (todosData.value = d))
}
...
get(id).then((d) => (userData.value = d))
getAllTodos(id).then((d) => (todosData.value = d))
...
function isClicked(todoData: Todo) {
  ...
  putTodo(id, newTodoData, newTodoData.id!.toString())
}
</script>
      ...
      <fwb-table-row v-for="todo in todosData" v-bind:key="todo.id">
        ...
        >
          <div class="flex flex-col -mx-6 text-center md:mx-0">
            ...
            <div class="flex justify-start font-normal text-slate-500">{{ todo.description }}</div>
          </div>
        </fwb-table-cell>
      </fwb-table-row>
    </fwb-table-body>
